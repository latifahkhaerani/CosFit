...
export default function CreateProd(){
    ...
    <section className="mx-auto max-w-3xl">
      <Link href="/vendor" className="mb-8 inline-flex items-center gap-2 text-sm font-semibold text-[var(--muted)] hover:text-[var(--primary)]">
        <ArrowLeft size={17} /> Back
      </Link> 

      <div className="card p-6 sm:p-10">
        <div className="mb-8">
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--primary)]">CosFit Community</p>
          <h1 className="mt-2">Create New Product</h1>
          <p className="mt-3 text-[var(--muted)]">Create product for you customers.</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
            <Field label="Stock" htmlFor="stock" icon={<Tags size={17} />}>
                <input id="stock" required className="input-soft w-full" placeholder="0" value={stock} onChange={(e) => {setStock(e.target.value)}} />
            </Field>
            <Field label="Price" htmlFor="price" icon={<Tags size={17} />} hint="Your product theme.">
                <input id="price" required className="input-soft w-full" placeholder="$0" value={originalPrice} onChange={(e) => {setOriginalPrice(e.target.value)}} />
            </Field>
        </div>

          {error ? <p role="alert" className="rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-700">{error.message}</p> : null}

          <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end">
            <Link href="/forum" className="secondary-btn inline-flex items-center justify-center">Cancel</Link>
            <button type="submit" disabled={isSubmitting} className="primary-btn inline-flex items-center justify-center gap-2 disabled:cursor-not-allowed disabled:opacity-60">
              <Plus size={18} /> {isSubmitting ? "Creating..." : "Create Product"}
            </button>
          </div>
        </form>
      </div>
